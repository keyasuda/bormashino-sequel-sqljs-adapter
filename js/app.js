...
const main = async () => {
  ...
  const db = dbWrapper(new SQL.Database())
  window.database = db
  db.db.run('CREATE TABLE test (col1, col2);')
  // Insert two rows: (1,111) and (2,222)
  db.db.run('INSERT INTO test VALUES (?,?), (?,?)', [1, 111, 2, 222])
  db.db.run('INSERT INTO test VALUES (?,?), (?,?)', [3, 333, 4, 444])
